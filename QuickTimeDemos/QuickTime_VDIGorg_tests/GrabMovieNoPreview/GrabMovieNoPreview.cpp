...
void doGrabMovieNoPreview()
{
	ofstream ofs("GrabMovieNoPreview.txt");
	
	cout << "GrabMovieNoPreview..." << endl;
	ofs << "GrabMovieNoPreview..." << endl;
	...
	CheckErr("SGSetFrameRate",::SGSetFrameRate(mVideoChannel, X2Fix(0))); // X2Fix(6.0)));
	...
	//	is encountered.
	//
	err = ::SGSetChannelMaxFrames(mVideoChannel,-1);
	cout << "SGSetChannelMaxFrames(-1)\t" << err << endl;
	ofs << "SGSetChannelMaxFrames(-1)\t" << err << endl;
	
	cout << "Capturing 8 seconds of video as fast as we can..." << endl;
	while (!err)
